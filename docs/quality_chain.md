...
**插件安全模式建议 secure-by-default**（r42+r46 audit 澄清）：
- `--sandbox-plugin` 启用 subprocess 隔离（r28 S-H-4）——推荐的默认安全
  模式。plugin 子进程独立 `sys.modules`，无法 monkey-patch host 的
  `core.lang_config.resolve_translation_field`（r42 checker deferred
  import 的潜在 supply-chain 面）或任何其他 host 模块
- **Legacy `importlib` 模式**（不传 `--sandbox-plugin`）：plugin 直接
  加载进 host 进程，可 monkey-patch 任何 host 函数。仅用于完全受信的
  first-party plugin；第三方 plugin 应**始终** `--sandbox-plugin`
