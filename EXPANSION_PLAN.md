...
**涉及文件**：`direct_translator.py`（`_translate_chunk_with_retry` 和 `_should_retry`）
...
**现状**：`tl_translator.py` 的 `_clean_rpyc` 函数已存在，但需要用户在 `_apply_tl_game_patches` 中手动触发。
...
**涉及文件**：`tl_translator.py`（`run_tl_pipeline` 尾部 + CLI 参数传递）
...
1. **Ren'Py 代码零改动**：现有 `direct_translator` / `tl_translator` / `retranslator` 不重构，通过薄包装类接入新架构
...
- `status` 复用现有的状态命名约定（与 translation_db 的 status 字段一致），便于统一报告
...
- 检查 `args.tl_mode` → 调用 `tl_translator.run_tl_pipeline(args)`
- 检查 `args.retranslate` → 调用 `retranslator.run_retranslate_pipeline(args)`
- 默认 → 调用 `direct_translator.run_pipeline(args)`

**import 策略**：`run()` 内部 deferred import `direct_translator` / `tl_translator` / `retranslator`，并 try-except 捕获 ImportError 给友好提示。
...
engine.run(args) ◄────────────────────────────────────────────────┘
  ...
  │     ├── direct_translator.run_pipeline()
  │     ├── tl_translator.run_tl_pipeline()
  │     └── retranslator.run_retranslate_pipeline()
  │
...
**处理策略**：第一版不自动处理字体。在翻译完成后的报告中提示用户需要手动安装中文字体，并给出常见的字体安装方法。后续可以做成可选的 `--patch-font` 功能（类似现有 Ren'Py 的 `font_patch.py`）。
...
RPG Maker 的 prompt 与 Ren'Py 有几个关键差异需要在 `prompts.py` 中体现：
...
**建议实现**：在 `prompts.py` 中新增一个 `_ENGINE_PROMPT_ADDONS` 字典，key 为 `EngineProfile.prompt_addon_key`，value 为追加到基础 prompt 末尾的引擎专属说明。Ren'Py 现有 prompt 零改动。
...
这个模块是所有非 Ren'Py 引擎共用的翻译编排器。它的流程类似于 `direct_translator.py` 的 `run_pipeline`，但基于 `TranslatableUnit` 而非 `.rpy` 文件。
...
3. 解析返回的 JSON 数组（复用 `api_client.parse_json_response` 的 6 级容错）
...
**重试策略**：复用 `api_client.py` 的指数退避 + Retry-After + jitter 机制。chunk 级重试暂不实现（留给后续迭代，可复用 Ren'Py 的 `_translate_chunk_with_retry` 逻辑）。
...
|----------|-------------------|-------------|
| `api_client.py` | 直接复用。`APIClient`、`RateLimiter`、`UsageStats`、`parse_json_response` | **不改** |
| `glossary.py` | 直接复用。`Glossary`、`load_dict`、`to_prompt_text`、`update_from_translations` | **小改**：新增 `scan_rpgmaker_database()` 方法 |
| `translation_db.py` | 直接复用。`TranslationDB` 存储 per-unit 翻译元数据 | **不改** |
| `prompts.py` | 新增引擎 addon 机制。`build_system_prompt` 新增 `engine_profile` 参数（可选，默认 None 走 Ren'Py 路径） | **小改**：新增 addon 字典 + 通用模板 |
...
| `prompts.py` | `_ENGINE_PROMPT_ADDONS` 字典 + `build_system_prompt(engine_profile=)` | ~40 行 | **+40 行** | 不变（baseline 测试保证） |
| `glossary.py` | `scan_rpgmaker_database()` 方法 | ~40 行 | **+60 行** | 不变 |
...
| 适配 `prompts.py` | 0.5 天 | ✅ 已完成（引擎 addon 机制） |
...
| `glossary.py` 新增 `scan_rpgmaker_database` | 0.5 天 | ✅ 已完成 |
| 测试 | 1 天 | ✅ 已完成（62 引擎测试） |
