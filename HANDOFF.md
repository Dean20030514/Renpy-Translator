# 交接笔记（第 33 轮结束 → 第 34 轮起点）

**目标读者**：下次新对话接手这个项目的 AI / 开发者

**当前时间锚点**：第 33 轮已完成。若您在新对话里看到这份文档，意味着上次工作结束于此处。

---

## 项目当前状态（一句话）

纯 Python 零依赖多引擎游戏汉化工具，**346 个自动化测试 + tl_parser 75 + screen 51 内建自测全绿**（共 472 断言点）。第 33 轮清掉 HANDOFF round 32 挂起的三项"🟢 自然延续 round 32 方向"小项：`tools/merge_translations_v2.py` 把多次 `--target-lang` 运行的 v2 envelope 合并成一个多语言文件、`--font-config` 现在透传到 runtime hook 生成 `zz_tl_inject_gui.rpy`（`init 999` 覆盖 gui 字号/布局）、`tools/translation_editor.py` 自动识别 v2 envelope 并通过 `--v2-lang` 支持按语言 bucket 编辑 + 写回。收尾时顺手拆分了 `tests/test_translation_state.py`（1256 → 504 行）把 21 条 runtime-hook 测试移到新的 `tests/test_runtime_hook.py`（791 行），测试套件数从 14 扩成 15。所有新功能 default off，既有 tl-mode / direct-mode / retranslate / screen / 静态 `--patch-font` 行为 byte-identical。
...
HANDOFF round 32 挂起的三项绿色小项（🟢）已全部完成于 round 33。Priority A + B 清零持续 5 轮；项目仍处于 steady-state。

### 🟢 自然延续 round 33 方向（小项）

**(1) `TranslationDB.entries` 加 `language` 字段**：现在 v2 schema 只在 emit 层拼装语言 bucket（`core/runtime_hook_emitter.py::build_translations_map`），TranslationDB 本身还是单语言存储。若要真实让同一次翻译运行 first-class 支持多语言输出（譬如同时产出 zh + ja），需要给 entry schema 加 `language` 字段 + 升级 upsert/has_entry/filter_by_status 的 `(file, line, original, language)` 四元组 key + save/load 加版本号兼容旧 DB。~3-5h 含数据迁移 + 向后兼容测试。

**(2) editor HTML **同页**多语言切换**：当前 `--v2-lang zh` 产一个 HTML，编辑完再跑 `--v2-lang ja` 产另一个 HTML。如果 v2 envelope 含 3+ 语言，这很繁琐。实现一个在 HTML 里切换语言显示/编辑的 UI（基于已有的 `v2_langs_seen` metadata）。~2-3h，纯 JS 工作。

**(3) `--font-config` 读 `api_engine_token` 风格的 subkey 支持**：目前只认 `gui_overrides`。若 font_config 未来扩展（eg `nvl_overrides` / `text_style_overrides`），需分类处理。本轮架构已经支持（`_sanitise_gui_overrides` 可拆成 generic helper）。~1-2h。
...
- CI Windows runner
- docs/constants.md / docs/quality_chain.md / docs/roadmap.md 深度复查（连续 4 轮被提及未做）
...
|------|------|---------|
| 分层违规 | ✅ 清零 | round 27 |
| 大文件（>800 行）| ✅ 零（源码最大 tests/test_runtime_hook.py 791；tools/translation_editor.py 749；translators/direct.py 601；checker.py 约 615；inject_hook.rpy 约 345） | round 33 |
...
| 运行时注入 | ✅ `--emit-runtime-hook` 基础（r31）+ UI whitelist sidecar / 字体 bundle / v2 多语言 schema（r32）+ gui_overrides 透传 / v2 merge 工具 / editor v2 编辑（r33）| round 33 |
| 字体路径解析 | ✅ 全部走 `core.font_patch.default_resources_fonts_dir()` canonical helper，2 处遗留 bug 已修 | round 32 |
| 生成代码注入防护 | ✅ `zz_tl_inject_gui.rpy` 生成走 `_SAFE_GUI_KEY` 正则 + `int`/`float` 白名单（拒 bool/str/list/dict/None），bool 单独过滤（避免 `isinstance(True, int)` 漏网）| round 33 |
| 潜伏 bug | ✅ 清零（round 28/29/30/32 陆续扫清）| round 32 |
| 测试覆盖 | ✅ **346 自动化** + tl_parser 75 + screen 51 = **472 断言点**；15 测试套件 | round 33 |
| 文档同步 | ✅ CLAUDE / .cursorrules / HANDOFF / CHANGELOG / TEST_PLAN / dataflow_pipeline / README / engine_guide 均现行 | round 33 |
...
| 本次交接 | `HANDOFF.md`（本文件） |
| 变更日志 | `CHANGELOG_RECENT.md`（含第 31/32/33 轮详细 + 第 1-30 轮摘要） |
...
| 测试 | `tests/test_all.py`（meta，6 模块）+ 6 个拆分（test_api_client / test_file_processor / test_translators / test_glossary_prompts_config / test_translation_state / **test_runtime_hook** r33 新）+ 9 个独立套件（test_engines / smoke_test / test_rpa_unpacker / test_rpyc_decompiler / test_lint_fixer / test_tl_dedup / test_batch1 / test_custom_engine / test_direct_pipeline / test_tl_pipeline / test_translation_editor / **test_merge_translations_v2** r33 新） |

---

**本文件由第 33 轮末尾自动生成，作为第 34 轮起点。**
