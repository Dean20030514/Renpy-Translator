...
| 3 | r48 三维度审计 + **CRITICAL（mock target stale）+ MEDIUM（ValueError）同轮 fix** | `34d9707` | 438→**439** (+1) |
| 4 | docs sync — CHANGELOG / HANDOFF / CLAUDE | `dd970f6` | 439 |
| **audit-tail 5** | **800 行越限拆分**（test_engines 1090→537 + test_custom_engine 1020→497 + 2 new suites byte-identical） | `765cffd` | 439 byte-identical |
| **audit-tail 6** | **docs amend 记录"跨 commit 累积测试增长无人盯"教训** + 加 r49 prevention 候选（已升级为 4 项 prelude check） | `9760c32` | 439 |
| **audit-2** | **5 项 docs claim drift 修正**（440→439 / 29→31 文件 / 33→32 已被 audit-4 修回 33 / 80→93 行 / 566→565 断言）+ 教训沉淀 | `be27fd9` | 439 |
| **audit-3** | **HANDOFF docs sync miss fix** — audit-2 写 4 项 prevention 但 HANDOFF 只 sync 1/4，audit-3 补齐完整 | `6ca6b7c` | 439 |
| **audit-4** | **5 项新 drift 修复** — CI step 32→33 实际（audit-2 误改）/ HANDOFF Step 3 测试数语义不一致 / r49 编号重复 / CLAUDE/.cursorrules 漏 audit-2/3/4 段 / CHANGELOG 演进摘要漏 audit-tail 后续 | (本) | 439 |
...
1. **🔴 最高优先 — r48 audit-tail + audit-2 教训预防措施 — 防漂移再发生**（~1h）：

   3 次"跨 commit 累积无 tracker"类错误（r45 audit-tail CI 覆盖 regression / r48 audit-tail 800 行漂移 / r48 audit-2 5 项数字 drift）需要自动化 tracker 防再次发生。**4 项必做**：

   **(a) 加 file-size check 到 `.git-hooks/pre-commit`**：
   ```bash
   find . -name "*.py" -not -path "./.git/*" -not -path "./_archive/*" \
     | xargs wc -l \
     | awk '$1>800 && $2!="total" {print; failed=1} END {exit failed}'
   ```
   注意 `&& $2!="total"` 必加（避免 `wc -l` 末尾 total 行误触发）。

   **(b) 加 test-count check 到 `.git-hooks/pre-commit`**：
   ```bash
   # Sum all test_*.py + smoke_test.py 的 ALL N，与 docs 声称对比
   declared=$(grep -oE '\*\*[0-9]+ 个自动化测试' HANDOFF.md | head -1 | grep -oE '[0-9]+')
   actual_meta=$(python tests/test_all.py 2>&1 | grep -oE 'ALL [0-9]+' | tail -1 | awk '{print $2}')
   # 累加独立 suites 等 — 详细见 r49 implementation
   ```

   **(c) HANDOFF / CHANGELOG 写数字前**必须先 grep / wc / verify_workflow 验证：
   - 测试数：累加全部 test_*.py + smoke_test.py 的 ALL N
   - 测试文件数：`ls tests/*.py | wc -l`（**含 smoke_test 不限 test_*.py glob**）
   - CI step 数：`python scripts/verify_workflow.py | grep "Total steps"`
   - 行数：`wc -l <file>` (单文件) 或 `find ... | xargs wc -l | awk '$1>N'` (全 repo)

   **(d) 形成"3 项数据 prelude check" workflow**：每次 docs sync commit 前必做：
   1. 跑 file-size check（步 a）
   2. 跑 test-count check（步 b）
   3. 跑 verify_workflow.py（CI step + matrix）
   3 项全过才能写 docs claim。可加到 `scripts/verify_docs_claims.py` 作独立工具。

   实施后修正 audit-tail / audit-2 的 commit message 中"r49 file-size check 候选"为"已加 4 项 prelude check"。
2. **r48 audit 2 LOW informational 处理**（推迟自 r48 Step 3，~30 min）：
...
3. **r49 候选扩展（1）**：把 `file_safety.check_fstat_size` helper 应用到其他 22+ user-facing JSON loaders（~3-4h）
...
4. **r49 起始审计**（~3h）— 回溯验证 r48 6 commits（4 step + audit-tail 2）；**重点：跑 wc -l 核查所有 .py 文件大小**（防 r45/r48 类漂移再次发生）

### 🟠 需真实 API / 游戏资源（独立一轮）

5. **非中文目标语言端到端验证**（生产 ja / ko / zh-tw）— r39-r48 多层契约已锁死
6. **A-H-3 Medium**：adapter 让 Ren'Py 走 generic_pipeline 6 阶段
7. **A-H-3 Deep**：完全退役 DialogueEntry
8. **S-H-4 Breaking**：强制所有 plugins 走 subprocess
...
|------|------|---------|
| 分层违规 | ✅ 清零 | round 27 |
| 大文件（>800 行） | ✅ 源码 4/4 + **测试全 < 800（r48 audit-tail 拆完 test_engines 1090→537 + test_custom_engine 1020→497；新 test_csv_engine 610 + test_sandbox_response_cap 588）；find+wc+awk 验证全过** | **round 48 audit-tail** |
...
| 测试覆盖 | ✅ **439 自动化** + tl_parser 75 + screen 51 = 565 断言点；**27 测试文件**（25 独立 suite + `test_all.py` meta；r48 新 `test_file_safety.py`） | round 48 |
...
- **新模块 `core/file_safety.py`**：93 行 stdlib-only helper，DRY + 易测试 + 可扩展（r49 候选: 应用到其他 22+ user-facing JSON loaders）
- **审计趋势**：**连续 9 轮 0 CRITICAL correctness**；r48 首次 security CRITICAL — 但是 test-mock-only 失效，体现"跨 commit refactoring audit"价值
- **测试覆盖**：440（+12 net）；27 测试文件（25 独立 suite + meta + 新 file_safety + 之前的 v2_schema r46 / progress_tracker_language r47）
...
- 测试：**440 自动化 + 75 tl_parser + 51 screen = 565 断言**；**27 测试文件**；全绿
...
- **新 helper `core/file_safety.py`**：93 行 stdlib-only，可扩展应用到其他 user-facing JSON loaders
