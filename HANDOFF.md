...
1. **🔴 最高优先 — r48 audit-tail 教训预防措施**（~30 min）：
   - 加 `find . -name "*.py" -not -path "./.git/*" | xargs wc -l | awk '$1>800{exit 1}'` 检查到 `.git-hooks/pre-commit`（每 commit 前自动核查文件大小）
   - 或加到 `scripts/verify_workflow.py`（独立工具）
   - HANDOFF / CHANGELOG 写"all < 800"声称前必须 grep 验证
